...
export const Logo: React.FC<LogoProps> = ({ logoUrl, appName = "ChuteFlix" }) => {
  if (logoUrl) {
    return (
      <Image
        src={logoUrl}
        alt={`${appName} Logo`}
        width={128} // Largura original para proporção
        height={32} // Altura original para proporção
        className="h-8 w-auto" // Controla o tamanho da exibição
      />
    );
  }
